...
app = Flask(__name__)
# التأكد من أن المجلد يتبع هيكلية Flask الصحيحة
...
def encrypt_start():
    ...
    if file:
        ...
        file.save(file_path)
        # نرسل المسار بالنسبة لمجلد static
...
def encrypt_action():
    ...
    # نرسل اسم الملف فقط والواجهة ستستخدم url_for
...
def receiver_upload():
    ...
    if file:
        ...
        # التعديل المهم هنا: نرسل الملفات للواجهة
...
def decrypt_action():
    ...
    dec_file = decrypt_image(os.path.join(UPLOAD_FOLDER, data['file']), data['key'])
    if dec_file:
